...
## ให้เจ้าภาพแก้ไข "จำนวนผู้มางาน" จากหน้า Dashboard ได้

**ต้องรัน SQL นี้ 1 ครั้งก่อนใช้งานฟีเจอร์นี้** — SQL Editor → New query → วางแล้ว Run:

```sql
alter table rsvp_responses
  add column if not exists edited_by_host boolean not null default false;
```

ไม่ต้องแก้ policy อะไรเพิ่ม (policy insert เดิมครอบคลุมคอลัมน์ใหม่นี้อยู่แล้ว) และแขกที่กรอกฟอร์มปกติจะได้ค่า `false` โดยอัตโนมัติ ไม่กระทบอะไร

กลไกเบื้องหลัง: ใช้หลักการเดียวกับ "แก้ไขคำตอบเดิม" ด้านบนทุกประการ — ตอนเจ้าภาพกดแก้ไขจำนวนแขกในหน้า Dashboard ระบบจะ **insert แถวใหม่** ที่มี `device_id` เดิมของแขกคนนั้น (คัดลอกชื่อ/สถานะมา-ไม่มา/ข้อความเดิมมา) เปลี่ยนแค่ `guests` ใหม่และติดธง `edited_by_host = true` แล้วให้ dedup logic ที่มีอยู่แล้วเลือกแถวนี้มาแสดงเป็นคำตอบล่าสุดโดยอัตโนมัติ พร้อมขึ้นป้าย "แก้ไขโดยเจ้าภาพ" กำกับไว้ในตาราง — **ไม่ใช้ UPDATE เลย** เพราะเจอบั๊กด้านบนมาแล้ว
